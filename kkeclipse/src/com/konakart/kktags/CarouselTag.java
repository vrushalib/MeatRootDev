	...
	public int doStartTag() throws JspException {
		try {
			...
			if (prods != null && prods.length > 0) {
				...
				sb.append("function slider" + rand
						+ "nextCallback(carousel,control,flag) {");
				sb.append("if (flag) {");
				sb.append("jQuery('#kk-forward-"
						+ rand
				...
				sb.append("function slider" + rand
						+ "prevCallback(carousel,control,flag) {");
				sb.append("if (flag) {");
				...
				sb.append("<div class=\"item-area " + width
						+ " rounded-corners\">");
				sb.append("<div class=\"item-area-header\" style=\"border-bottom: 1px solid #6e6963;\">");
				sb.append("<h2 class=\"item-area-title\">");
				if (prods[0].getCategoryId() > 0) {
					sb.append("<a href=\"SelectCat.action?catId="
							+ prods[0].getCategoryId() + "\">");
					sb.append(title);
					sb.append("</a>");
				} else {
					sb.append(title);
				...
				sb.append("<span id=\"kk-back-" + rand
						+ "\" class=\"item-arrow\"></span>");
				sb.append("<span class=\"separator\"></span>");
				sb.append("<span id=\"kk-forward-" + rand
						+ "\" class=\"item-arrow\"></span>");
				sb.append("</div>");
				sb.append("</div>");

				sb.append("<div id=\"slider" + rand
						+ "\" class=\"items jcarousel-skin-kk\">");
				sb.append("<ul>");
