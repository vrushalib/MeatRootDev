...
public class EditNotifiedProductsSubmitAction extends BaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
            ...
            if (itemList != null)
            {
                for (Iterator<NotifiedProductItem> iterator = itemList.iterator(); iterator
                        .hasNext();)
                {
                    NotifiedProductItem item = iterator.next();
                    if (item.isRemove())
                    {
            ...
            int gNotification = (isGlobalNotificationBool()) ? 1 : 0;
